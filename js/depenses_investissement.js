(function(d3) {
  ...
  var pie = d3.pie()
    .value(function(d) { return d.dep_invest; })
    .sort(null);

  d3.csv('dep_invest.csv', function(error, dataset) {
    dataset.forEach(function(d) {
      d.dep_invest = +d.dep_invest;
      d.enabled = true;
    });
    ...
    var somme_df = d3.sum(dataset, function(d) {
            ...
            var glurb = g.append("foreignObject")
                ...
                .attr("height", 100)
                .append("xhtml:h5")
                .html("DÉPENSES D'INVESTISSEMENT <br>" + (d3.format(",")(somme_df).replace(/,/g, ' ')) + " €");
                ...
                var path = svg.selectAll('path')
                  ...
                  .on("mouseover", function(d,i) {
                   ...
                   .on("mouseout", function(d) {
                     glurb.html("DÉPENSES D'INVESTISSEMENT <br>" + (d3.format(",")(somme_df).replace(/,/g, ' ')) + " €")
                        .style('color', "#555");
                   })
                  .each(function(d) { this._current = d; });

    var aargh = d3.select("#legend2")
...
var maxtextWidth = Math.max(...textWidth);
                ...
                aargh.selectAll("div")
                     ...
                     .style('class','a')
                      ...
                      .attr('data-target',function(d,i){return "#depinvest" + i;});

                      aargh.selectAll("div")
                            .attr('class','explain')
                            .append("text")
                            .attr('class','collapse')
                            .attr('id',function(d,i){return "depinvest" + i;})
                            .append('p')
                            .style('margin','5px 0px 5px 10px')
                            .attr('class','h3')
                            .style('font-weight','900')
                            .style('color', function(d){return color(d.data.label);})
                            .html(function(d){return (d3.format(",")(d.data.dep_invest).replace(/,/g, ' ')) + " €";});

                      aargh.selectAll("text")
                            .append('p')
                            .style('margin-left','10px')
                            .html(function(d){return (d.data.dep_descr);});
